# python train.py --checkpoints ./CelebA_Pconv --model 2
...
class MaskPerceptionLayer(nn.Module):
    def __init__(self, inputChannels, outputChannels, kernelSize, stride,
                 padding, dilation=1, groups=1, bias=False):
        super(MaskPerceptionLayer, self).__init__()

        self.conv = nn.Conv2d(inputChannels, outputChannels, kernelSize, stride, padding, dilation, \
                              groups, bias)
        self.conv1_1 = nn.Conv2d(outputChannels, outputChannels, 1, 1)

        if inputChannels == 4:
            self.maskConv = nn.Conv2d(1, outputChannels, kernelSize, stride, padding, dilation, \
                                      groups, bias)
        else:
        ...
        if inputChannels == 4:
            self.maskConv2 = nn.Conv2d(2, 2, kernelSize, stride, padding, dilation, groups, bias)
        else:
            self.maskConv2 = nn.Conv2d(inputChannels, 2, kernelSize, stride, padding, dilation, groups, bias)
    ...
    def forward(self, inputFeatures, inputMasks):
        ...
        if inputMasks.shape[1] != 1:
            maskFeatures = self.maskConv2(inputMasks)
            # print('maskFeature:')
            # print(maskFeatures.shape)
            maskFeatures = F.softmax(maskFeatures.permute(0, 2, 3, 1), dim=-1).argmax(axis=-1).float()
            # print(maskFeatures.shape)
            maskFeatures = maskFeatures.view((maskFeatures.shape[0], 1, maskFeatures.shape[1], maskFeatures.shape[2]))
            # print(maskFeatures.shape)
        else:
            maskFeatures = inputMasks
        maskActiv = self.activationFuncG_A(maskFeatures)
        convOut = convFeatures * maskActiv

        return convOut, maskFeatures, convFeatures, maskActiv


# forward attention gather feature activation and batchnorm
class MaskPerceptionAttention(nn.Module):
    def __init__(self, inputChannels, outputChannels, bn=True, sample='down-4', \
                 activ='leaky', convBias=False):
        super(MaskPerceptionAttention, self).__init__()

        if sample == 'down-4':
            self.conv = MaskPerceptionLayer(inputChannels, outputChannels, 4, 2, 1, bias=convBias)
        elif sample == 'down-5':
            self.conv = MaskPerceptionLayer(inputChannels, outputChannels, 5, 2, 2, bias=convBias)
        elif sample == 'down-7':
            self.conv = MaskPerceptionLayer(inputChannels, outputChannels, 7, 2, 3, bias=convBias)
        elif sample == 'down-3':
            self.conv = MaskPerceptionLayer(inputChannels, outputChannels, 3, 2, 1, bias=convBias)
        else:
            self.conv = MaskPerceptionLayer(inputChannels, outputChannels, 7, 1, 3, bias=convBias)
        if bn:
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, residual_blocks=7, init_weights=True):
        ...
        self.ec1 = MaskPerceptionAttention(4, 64, sample='')
        self.ec2 = MaskPerceptionAttention(64, 128, sample='down-4')
        self.ec3 = MaskPerceptionAttention(128, 256, sample='down-4')
    ...
    def forward(self, x, mask_whole, m1, m2):
        # m1:64*256*256 fe1:64*256*256
        # m2:128*128*128 fe2:128*128*128
        # mask_whole:1*256*256

        f_e1, mu1, _, _ = self.ec1(x, mask_whole)
        # f_e1 = self.encoder1(x)
        print(f_e1.shape)
        f_e2, mu2, _, _ = self.ec2(f_e1, m1)
        # f_e2 = self.encoder2(f_e1)
        print(f_e2.shape)
        f_e3, mu3, _, _ = self.ec3(f_e2, m2)
        # f_e3 = self.encoder3(f_e2)
        print(f_e3.shape)
        x = self.middle(f_e3)
        print(x.shape)
        x, _ = self.auto_attn(x, f_e3, mu3)
        print(x.shape)
        x = self.decoder1(x)
        print(x.shape)
        x = self.fushion1(torch.cat((f_e2, x), dim=1))
        print(x.shape)
        x = self.decoder2(x)
        print(x.shape)
        x = self.fushion2(torch.cat((f_e1, x), dim=1))
        print(x.shape)
        x = self.decoder3(x)
        print(x.shape)
        x = (torch.tanh(x) + 1) / 2
...
if __name__ == '__main__':
    Inpaintnet = InpaintGenerator()
    inp = torch.rand((4, 3, 256, 256))
    l = torch.rand((4, 1, 256, 256))
    inputs = torch.cat((inp, l), dim=1)
    # print(inputs.shape)
    m = torch.rand((4, 1, 256, 256))
    m_q = F.interpolate(m, size=[int(m.shape[2] / 4), int(m.shape[3] / 4)],
                        mode='bilinear', align_corners=True)
    # print(m_q.shape)
    m_h = F.interpolate(m, size=[int(m.shape[2] / 2), int(m.shape[3] / 2)],
                        mode='bilinear', align_corners=True)
    # print(m_h.shape)
    m1 = torch.rand((4, 64, 256, 256))
    m2 = torch.rand((4, 128, 128, 128))
    # summary(Inpaintnet.cuda(),[(4,256,256),(1,256,256),(1,128,128),(1,64,64)])
    x = Inpaintnet(inputs, m, m_h, m_q, m1, m2)

    # print("space layer:")
    # input2=torch.rand((4,16,256,256))
    # sl=SpatialAttentionModule()
    # input2=sl(input2)
    # print(input2.shape)
