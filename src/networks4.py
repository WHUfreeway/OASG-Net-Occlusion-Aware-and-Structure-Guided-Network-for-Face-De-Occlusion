...
class MaskPerceptionLayer(nn.Module):
    def __init__(self, inputChannels, outputChannels, kernelSize, stride,
        ...
        self.imageAttention=SELayer(outputChannels)
    ...
    def forward(self, inputFeatures, inputMasks):
        ...
        maskFeatures2=maskFeatures.clone()
        maskFeatures2=self.conv1_1(maskFeatures2)
        maskActiv = self.activationFuncG_A(maskFeatures2)

        convOut = convFeatures * maskActiv
        convOut=  self.imageAttention(convOut)
        maskUpdate = self.updateMask(maskFeatures)
...
class InpaintGenerator(BaseNetwork):
    ...
    def forward(self, x, mask_whole, mask_half, mask_quarter):
        ...
        #print(f_e1.shape)
        f_e2, mu2, _, _ = self.ec2(f_e1, mu1)
        # f_e2 = self.encoder2(f_e1)
        #print(f_e2.shape)
        f_e3, mu3, _, _ = self.ec3(f_e2, mu2)
...
if __name__ == '__main__':
    ...
    x = Inpaintnet(inputs, m, m_h, m_q)

    #print("selayer:")
    #input2=torch.rand((4,16,256,256))
    #selayer=SELayer(channel=input2.shape[1])
    #input2=selayer(input2)
    #print(input2.shape)
