...
from .utils import Progbar, create_dir, stitch_images, imsave, imshow
...
class Lafin():
    def __init__(self, config):
        ...
        if self.config.MODE == 1:
            if self.config.MODEL == 1:
                self.train_dataset = Dataset(config, config.TRAIN_LANDMARK_IMAGE_FLIST,
                                             config.TRAIN_LANDMARK_LANDMARK_FLIST, config.TRAIN_MASK_FLIST,
                                             augment=True, training=True)
                self.val_dataset = Dataset(config, config.TEST_LANDMARK_IMAGE_FLIST,
                                           config.TEST_LANDMARK_LANDMARK_FLIST, config.TEST_MASK_FLIST, augment=True,
                                           training=True)
                self.sample_iterator = self.val_dataset.create_iterator(config.SAMPLE_SIZE)

            elif self.config.MODEL == 2:

                self.train_dataset = Dataset(config, config.TRAIN_INPAINT_IMAGE_FLIST,
        ...
        if self.config.MODE == 2:
            if self.config.MODEL == 1:
                self.test_dataset = Dataset(config, config.TEST_LANDMARK_IMAGE_FLIST,
                                            config.TEST_LANDMARK_LANDMARK_FLIST, config.TEST_MASK_FLIST, augment=False,
                                            training=False)

            else:
                self.test_dataset = Dataset(config, config.TEST_INPAINT_IMAGE_FLIST, config.TEST_INPAINT_LANDMARK_FLIST,
            ...
            for items in train_loader:
                    ...
                    if self.config.AUGMENTATION_TRAIN == 1:
                        ...
                        if self.train_dataset.augment == True:
                            if self.config.CROP_INPUT == 1:
                                X1, X2, Y1, Y2 = torch.min(landmarks_orig[..., 0], dim=1), torch.max(
                                    landmarks_orig[..., 0], dim=1), torch.min(landmarks_orig[..., 1], dim=1), torch.max(
                                    landmarks_orig[..., 1], dim=1)
                                ...
                                images_gen = F.interpolate(images_gen, [images.shape[2], images.shape[3]],
                                                           mode='bilinear')
                            ...
                            if np.random.uniform(0, 1) <= 0.2:
                                images_gen = F.interpolate(images_gen, [int(self.config.INPUT_SIZE * 3 / 8),
                                                                        int(self.config.INPUT_SIZE * 3 / 8)],
                                                           mode='bilinear')
                                images_gen = F.interpolate(images_gen, [self.config.INPUT_SIZE, self.config.INPUT_SIZE],
                                                           mode='bilinear')
                            for i in range(3):
                        ...
                        landmarks_output2, loss, logs2 = self.landmark_model.process_aug(images_gen, masks,
                                                                                         landmarks_orig)
                        self.landmark_model.backward(loss)
                ...
                # inpaint model
                    ...
                    landmark_map = torch.zeros(
                        (self.config.BATCH_SIZE, 1, self.config.INPUT_SIZE, self.config.INPUT_SIZE)).cuda()

                    for i in range(landmarks.shape[0]):
                        landmark_map[i, 0, landmarks[i, 0:self.config.LANDMARK_POINTS, 1], landmarks[i,
                                                                                           0:self.config.LANDMARK_POINTS,
                                                                                           0]] = 1
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint model
                ...
                iteration = self.inpaint_model.iteration
                # print("inpaint model iteration", iteration)
                ...
                landmark_mask_image = (landmark_mask_image.squeeze().cpu().numpy().transpose(1, 2, 0) * 255).astype(
                    'uint8')
                landmark_mask_image = landmark_mask_image.copy()
                for i in range(landmarks.shape[1]):
                    x = int(landmarks[0, i, 0].item())
                    y = int(landmarks[0, i, 1].item())
                    circle(landmark_mask_image, (x, y), radius=2, color=(0, 255, 0), thickness=-1)

                masked_images = self.postprocess(images * (1 - masks) + masks)[0]
                # images_result = self.postprocess(outputs_merged)[0]
                images_result = self.postprocess(outputs);
            ...
            # inpaint with joint model
                ...
                # print(output_landmark)
                # output_landmark=torch.Tensor([[4.0,112.0,11.0,138.0,14.0,161.0,20.0,183.0,33.0,209.0,50.0,229.0,72.0,245.0,98.0,254.0,124.0,261.0,147.0,251.0,157.0,241.0,166.0,232.0,173.0,212.0,179.0,186.0,182.0,167.0,186.0,144.0,186.0,118.0,53.0,96.0,69.0,86.0,85.0,86.0,98.0,86.0,111.0,93.0,153.0,89.0,163.0,86.0,173.0,83.0,179.0,83.0,182.0,89.0,134.0,122.0,137.0,141.0,140.0,157.0,140.0,170.0,118.0,180.0,124.0,183.0,134.0,183.0,140.0,183.0,147.0,180.0,72.0,118.0,85.0,115.0,95.0,115.0,105.0,122.0,95.0,125.0,82.0,125.0,147.0,118.0,157.0,112.0,169.0,109.0,173.0,112.0,166.0,118.0,157.0,122.0,101.0,212.0,114.0,206.0,127.0,199.0,134.0,203.0,140.0,199.0,150.0,206.0,153.0,209.0,150.0,219.0,144.0,222.0,134.0,225.0,124.0,225.0,114.0,219.0,101.0,212.0,124.0,209.0,134.0,209.0,140.0,209.0,153.0,209.0,140.0,212.0,134.0,212.0,124.0,212.0]])
                # print(output_landmark)

                landmark_pred = (output_landmark.detach()).reshape(-1, self.config.LANDMARK_POINTS, 2).long()
                # print(type(landmark_pred))

                landmark_pred[landmark_pred >= self.config.INPUT_SIZE - 1] = self.config.INPUT_SIZE - 1
                landmark_last_map = self.generate_landmark_map(landmark_pred, self.config.INPUT_SIZE)
                # print(landmark_last_map)

                images_out = self.inpaint_model(images, landmark_last_map, masks)
                # images_out, gen_loss, dis_loss, logs = self.inpaint_model.process(images, landmark_last_map, masks)

                outputs_merged = (images_out * masks) + (images * (1 - masks))
                # outputs_merged =images_out
                ...
                pure_landmark_image = (pure_landmark_image[0].cpu().numpy().copy().transpose(1, 2, 0) * 255).astype(
                    'uint8')
                pure_landmark_image = pure_landmark_image.copy()

                landmark_mask_image = images * (1 - masks) + masks
                landmark_mask_image = (landmark_mask_image[0].cpu().numpy().copy().transpose(1, 2, 0) * 255).astype(
                    'uint8')
                landmark_mask_image = landmark_mask_image.copy()
                for i in range(landmark_pred.shape[1]):
                    x = int(landmark_pred[0, i, 0].item())
                    y = int(landmark_pred[0, i, 1].item())
                    circle(landmark_mask_image, (x, y), radius=2, color=(0, 255, 0), thickness=-1)
                    circle(pure_landmark_image, (x, y), radius=2, color=(0, 255, 0), thickness=-1)
    ...
    def sample(self, it=None):
        ...
        if model == 1:
            ...
            landmarks_pred = (self.landmark_model(images * (1 - masks) + masks, masks)).reshape(
                (-1, self.config.LANDMARK_POINTS, 2)).long()
        ...
        # inpaint model
            ...
            for i in range(landmarks.shape[0]):
                landmark_map[
                    i, 0, landmarks[i, 0:self.config.LANDMARK_POINTS, 1], landmarks[i, 0:self.config.LANDMARK_POINTS,
                                                                          0]] = 1
            iteration = self.inpaint_model.iteration
            inputs = (images * (1 - masks)) + masks
            for i in range(inputs.shape[0]):
                inputs[i, :, landmarks[i, 0:self.config.LANDMARK_POINTS, 1],
                landmarks[i, 0:self.config.LANDMARK_POINTS, 0]] = 1 - masks[
                    i, 0, landmarks[i, :, 1], landmarks[i, :, 0]]
    ...
    def generate_landmark_map(self, landmark_cord, img_size=256):
        ...
        for i in range(landmark_cord.shape[0]):
            # 检查坐标是否在有效范围内
            valid_rows = (0 <= landmark_cord[i, :, 1]) & (landmark_cord[i, :, 1] < img_size)
            valid_cols = (0 <= landmark_cord[i, :, 0]) & (landmark_cord[i, :, 0] < img_size)
    
            # 如果有无效的坐标，打印出来
            if not (valid_rows.all() and valid_cols.all()):
                print("Invalid coordinates found for batch index:", i)
                print("Invalid row coordinates:", landmark_cord[i, ~valid_rows, 1])
                print("Invalid column coordinates:", landmark_cord[i, ~valid_cols, 0])
                continue  # 跳过这个batch的处理
        '''
